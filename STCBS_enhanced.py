...
from USTRRTstar import USTRRRTstar
        ...
        while conflict_tree:
            ...
            for agent in [conflict.agent1, conflict.agent2]:
                ...
                for invalid_node in invalid_nodes:
                    self.trees[agent].invalidate_node(invalid_node)
                ...
                for invalid_node in invalid_nodes:
                    if invalid_node.is_conflict:
                        invalid_node.is_conflict = False
                    self.trees[agent].validate_node(invalid_node)
    ...
    def coordinate_solution(self, high_level_node: HighLevelNode):
        ...
        for i in range(self.robot_num):
            while len(high_level_node.solution[i]) < max_time_step:
                ...
                self.trees[i].node_list.append(new_node)
